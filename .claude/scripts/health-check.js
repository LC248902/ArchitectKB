...
class VaultHealthCheck {
  ...
  calculateMetrics() {
    ...
    for (const note of this.notes) {
      ...
      // Count by type
      this.metrics.notesByType[type] =
        (this.metrics.notesByType[type] || 0) + 1;
      ...
      if (note.frontmatter.tags && note.frontmatter.tags.length > 0) {
        ...
        for (const tag of note.frontmatter.tags) {
          this.metrics.tagUsage.set(
            tag,
            (this.metrics.tagUsage.get(tag) || 0) + 1,
          );
        }
      }
      ...
      if (
        ...
        ["Task", "Project", "Adr"].includes(type)
      ) {
        if (!this.metrics.statusBreakdown[type]) {
          this.metrics.statusBreakdown[type] = {};
        }
        const status = note.frontmatter.status;
        this.metrics.statusBreakdown[type][status] =
          (this.metrics.statusBreakdown[type][status] || 0) + 1;
      }
      ...
      if (note.frontmatter.priority && ["Task", "Project"].includes(type)) {
        ...
        this.metrics.priorityBreakdown[type][priority] =
          (this.metrics.priorityBreakdown[type][priority] || 0) + 1;
      }
    }
    ...
    this.metrics.recentNotes.sort((a, b) =>
  ...
  isSpecialNote(note) {
    ...
    // Types that are expected to have no backlinks (not true orphans)
    const excludedTypes = [
      "Weblink", // External links - standalone by design
      "DailyNote", // Journal entries - standalone
      "Canvas", // Visual files
      "CodeSnippet", // Code examples
      "CodeSnipet", // Typo variant
      "Query", // Dataview queries
      "Course", // External learning content
      "AtomicNote", // Standalone atomic notes
      "Prompt", // AI prompts
      "Template", // Note templates
    ];

    // Pages and Tasks with tags are discoverable via Dataview, not true orphans
    const taggedDiscoverableTypes = ["Page", "Task"];
    const hasDiscoverableTags =
      taggedDiscoverableTypes.includes(type) && tags.length > 0;

    // Exclude templates, MOCs, dashboards, READMEs, standalone types, and tagged Pages/Tasks
    return (
  ...
  generateConsoleReport() {
    ...
    lines.push("");
    lines.push(
      chalk.gray(`Total Notes: ${chalk.white(this.metrics.totalNotes)}`),
    );
    lines.push("");
    ...
    lines.push("");
    const sortedTypes = Object.entries(this.metrics.notesByType).sort(
      (a, b) => b[1] - a[1],
    );
    ...
    lines.push(
      chalk.gray(
        `  Total Links:        ${chalk.white(this.metrics.linkStats.totalLinks)}`,
      ),
    );
    lines.push(
      chalk.gray(
        `  Total Backlinks:    ${chalk.white(this.metrics.linkStats.totalBacklinks)}`,
      ),
    );
    lines.push(
      chalk.gray(
        `  Avg Links/Note:     ${chalk.white(this.metrics.linkStats.averageLinksPerNote)}`,
      ),
    );
    ...
    if (this.metrics.linkStats.brokenLinks > 0) {
      lines.push(
        chalk.red(
          `  Broken Links:       ${this.metrics.linkStats.brokenLinks}`,
        ),
      );
    } else {
    ...
    lines.push("");
    const tagsPercent = (
      (this.metrics.qualityMetrics.notesWithTags / this.metrics.totalNotes) *
      100
    ).toFixed(1);
    const descPercent = (
      (this.metrics.qualityMetrics.notesWithDescription /
    ...
    lines.push(
      chalk.gray(
        `  Notes with tags:        ${chalk.white(this.metrics.qualityMetrics.notesWithTags)} (${tagsPercent}%)`,
      ),
    );
    lines.push(
      chalk.gray(
        `  Notes with description: ${chalk.white(this.metrics.qualityMetrics.notesWithDescription)} (${descPercent}%)`,
      ),
    );

    if (this.metrics.qualityMetrics.totalADRs > 0) {
      const adrQualityPercent = (
        (this.metrics.qualityMetrics.adrWithQualityIndicators /
          this.metrics.qualityMetrics.totalADRs) *
        100
      ).toFixed(1);
      lines.push(
        chalk.gray(
          `  ADRs with quality indicators: ${chalk.white(this.metrics.qualityMetrics.adrWithQualityIndicators)}/${this.metrics.qualityMetrics.totalADRs} (${adrQualityPercent}%)`,
        ),
      );
    }
    lines.push("");
    ...
    // Status breakdown
    if (
      Object.keys(this.metrics.statusBreakdown.Task || {}).length > 0 ||
      Object.keys(this.metrics.statusBreakdown.Project || {}).length > 0
    ) {
      lines.push(chalk.blue.bold("📋 Status Breakdown"));
      lines.push("");

      for (const [type, statuses] of Object.entries(
        this.metrics.statusBreakdown,
      )) {
        if (Object.keys(statuses).length > 0) {
          lines.push(chalk.gray(`  ${type}s:`));
          for (const [status, count] of Object.entries(statuses)) {
            lines.push(
              chalk.gray(`    ${status.padEnd(12)} ${chalk.white(count)}`),
            );
          }
        }
      }
    ...
    if (this.metrics.topTags && this.metrics.topTags.length > 0) {
      ...
      for (const { tag, count } of this.metrics.topTags.slice(0, 10)) {
        ...
        lines.push(
          chalk.gray(
            `  ${tag.padEnd(30)} ${chalk.cyan(count.toString().padStart(3))} ${bar}`,
          ),
        );
      }
      lines.push("");
    ...
    if (this.metrics.orphanedNotes.length > 0) {
      lines.push(
        chalk.yellow.bold(
          `⚠️  Orphaned Notes (${this.metrics.orphanedNotes.length})`,
        ),
      );
    ...
    if (this.metrics.staleNotes.length > 0) {
      lines.push(
        chalk.yellow.bold(`📅 Stale Notes (${this.metrics.staleNotes.length})`),
      );
      lines.push(chalk.gray(`  (Not modified in ${STALE_DAYS} days)\n`));

      for (const note of this.metrics.staleNotes.slice(0, 10)) {
        lines.push(
          chalk.yellow(`  - ${note.title} (${note.daysAgo} days ago)`),
        );
        lines.push(chalk.gray(`    ${note.path}`));
      }
  ...
  generateMarkdownReport() {
    ...
    lines.push(
      `- **Recent Activity:** ${this.metrics.recentNotes.length} notes (last 30 days)`,
    );
    lines.push("");
    ...
    lines.push(
      `| Total Backlinks | ${this.metrics.linkStats.totalBacklinks} |`,
    );
    lines.push(
      `| Average Links per Note | ${this.metrics.linkStats.averageLinksPerNote} |`,
    );
    ...
    lines.push(
      `- **Notes with tags:** ${this.metrics.qualityMetrics.notesWithTags} (${tagsPercent}%)`,
    );
    lines.push(
      `- **Notes with description:** ${this.metrics.qualityMetrics.notesWithDescription} (${descPercent}%)`,
    );
    ...
    if (this.metrics.qualityMetrics.totalADRs > 0) {
      ...
      lines.push(
        `- **ADRs with quality indicators:** ${this.metrics.qualityMetrics.adrWithQualityIndicators}/${this.metrics.qualityMetrics.totalADRs} (${adrQualityPercent}%)`,
      );
    }
    lines.push("");
    ...
    if (this.metrics.staleNotes.length > 0) {
      ...
      for (const note of this.metrics.staleNotes.slice(0, 20)) {
        lines.push(
          `- [[${note.title}]] - ${note.daysAgo} days ago (${note.modified})`,
        );
      }
      lines.push("");
    }
  ...
  calculateHealthScore() {
    ...
    const orphanRate =
      this.metrics.orphanedNotes.length / this.metrics.totalNotes;
    ...
    const recentRate =
      this.metrics.recentNotes.length / this.metrics.totalNotes;
    ...
    const brokenLinkRate =
      this.metrics.linkStats.brokenLinks /
      Math.max(this.metrics.linkStats.totalLinks, 1);
    ...
    const statusRate =
      this.metrics.qualityMetrics.notesWithStatus / this.metrics.totalNotes;
    const adrQualityRate =
      this.metrics.qualityMetrics.totalADRs > 0
        ? this.metrics.qualityMetrics.adrWithQualityIndicators /
          this.metrics.qualityMetrics.totalADRs
        : 0;
    ...
    score.total =
      score.connectivity + score.freshness + score.quality + score.completeness;
  ...
  formatHealthScore(score) {
    ...
    const lines = [];
    lines.push(
      getColor(score.total, 100)(`  Overall Score: ${score.total}/100`),
    );
    lines.push("");
    ...
    lines.push(
      getColor(
        score.connectivity,
        25,
      )(`    Connectivity:  ${score.connectivity}/25`),
    );
    lines.push(
      getColor(score.freshness, 25)(`    Freshness:     ${score.freshness}/25`),
    );
    lines.push(
      getColor(score.quality, 25)(`    Quality:       ${score.quality}/25`),
    );
    lines.push(
      getColor(
        score.completeness,
