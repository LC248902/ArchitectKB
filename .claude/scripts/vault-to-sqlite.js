 ...
 *   sqlite3 .data/vault.db "SELECT path, snippet(fts_content,1,'→','←','...',30) FROM fts_content WHERE fts_content MATCH 'kafka'"
 *
 * @see [[Incubator - Vault Architecture Evolution for Claude-First Workflows]]
 */
...
// Files to exclude
const EXCLUDED_FILES = ["README.md", "CLAUDE.md", "ARCHITECTURE-*.md"];
...
Query Examples:
  ...
  sqlite3 .data/vault.db -markdown "SELECT path, snippet(fts_content,1,'→','←','...',40) as match FROM fts_content WHERE fts_content MATCH 'kafka integration' ORDER BY rank LIMIT 10"

  # Find notes by tag
  sqlite3 .data/vault.db "SELECT n.path, n.title FROM notes n JOIN tags t ON n.id = t.note_id WHERE t.tag = 'technology/aws'"

  # Find backlinks to a note
  sqlite3 .data/vault.db "SELECT n.path FROM notes n JOIN links l ON n.id = l.source_id WHERE l.target_path LIKE '%Project - Alpha%'"
...
async function main() {
  ...
  console.log(
    "  sqlite3 .data/vault.db -markdown \"SELECT path, snippet(fts_content,1,'→','←','...',40) FROM fts_content WHERE fts_content MATCH 'kafka' LIMIT 10\"",
  );
}
