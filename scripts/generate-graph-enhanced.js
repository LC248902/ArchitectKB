...
class EnhancedGraphGenerator {
   ...
   */
  async buildPdfNodes() {
    const pdfOutputDir = path.join(VAULT_ROOT, "+PDFs", "docling_output");
    ...
    for (const file of doclingFiles) {
      try {
        ...
        const relativePath = path.relative(
          VAULT_ROOT,
          path.join(VAULT_ROOT, "+PDFs", pdfFilename),
        );

        // Title overrides for PDFs with poor auto-extracted titles
        // Add entries here mapping basename (without .pdf) to desired display title
        // Example: { "mybook2e": "My Book (2nd Edition)" }
